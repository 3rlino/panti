...
if (isset($_POST["update"])) {
  ...
  $query_get_old_data = "SELECT * FROM pembayaran WHERE id_pembayaran='$id_pembayaran'";
  ...
  $row_old_data = mysqli_fetch_assoc($result_old_data);
    ...
    if (
        ...
        $query = "UPDATE pembayaran SET kode_pembelian='$kode_pembelian', nama='$nama', tanggal='$tanggal', kode_pembayaran='$kode_pembayaran' WHERE id_pembayaran='$id_pembayaran'";
        if (mysqli_query($conn, $query)) {
            $_SESSION['data_edit'] = true;
            header("Location: pembayaran.php?editSuccess=1");
            exit();
        } else {
        ...
        header("Location: pembayaran.php"); // Redirect tanpa pesan sukses
        exit();
    }
        ...
        if (isset($_GET["id_pembayaran"])) {
            ...
            while($row = mysqli_fetch_array($result)) {
                ...
                <input type="hidden" name="id_pembelian" value="<?php echo $row['id_pembelian']; ?>">
                    ...
                    <textarea type="text" id="kode_pembelian" name="kode_pembelian" class="form-textarea" rows="1" cols="70" required><?php echo $row['kode_pembelian']; ?></textarea>
                    ...
                    <input type="text" id="kode_pembayaran" name="kode_pembayaran" class="form-input" value="<?php echo $row['kode_pembayaran']; ?>" required>
