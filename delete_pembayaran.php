...
function deletePembayaran($employee_id) {
    ...
    $query = "DELETE FROM pembayaran WHERE id_pembayaran = $employee_id";

    // Eksekusi query
    if (mysqli_query($connect, $query)) {
        $_SESSION['data_deleted'] = true;
            header("Location: pembayaran.php?deleteSuccess=1");
            exit();
    } else {
