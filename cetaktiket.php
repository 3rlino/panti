...
<body>
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-12">
                <br>
              <center><h1>Data Cetak Tiket</h1></center>
                <br>
                <div class="btn-box">
                <a href="admin_tambah_cetak.php" class="btn float-right">Tambah Data</a>
                </div>
            </div>
            <br />
            ...
            <table class="table-responsive table-striped">
                <tr>
                    <th style="width: 14%; padding: 10px;">Id Cetak</th>
                ...
                $query = "SELECT * FROM cetak ORDER BY id_cetak DESC LIMIT $limit_startt, $limitt";
                $result = mysqli_query($connect, $query);
                while($row = mysqli_fetch_array($result))
                {
                    echo '<tr>
                            <td style="text-align: center; padding: 10px;">'.$row["id_cetak"].'</td>
                            ...
                            <td><center><a href="ubah_cetak.php?id_cetak='.$row["id_cetak"].'" class="btnn btn-warning btn-xs"><i class="fas fa-edit"></i></a></center></td>
                            <td><center><a href="delete_cetak.php?employee_id='.$row["id_cetak"].'" class="btnn btn-danger btn-xs"><i class="fas fa-trash-alt" onclick="return confirm(\'Anda yakin ingin menghapus data ini?\')"></i></a></center></td>
                        </tr>';
                }
                ?>
            </table>
            ...
            <?php
            $query_jumlah = "SELECT count(*) AS jumlah FROM cetak";
...
<?php else : ?>
    <section id="home" class="home">
        ...
        <div class="col-md-6"><br>
            <h1>Cetak Tiket</h1>&nbsp;
            <form id="cetakForm" method="POST" action="proses_cetak_tiket.php" enctype="multipart/form-data" target="_blank">

                <input type="hidden" id="id_pembayaran" name="id_pembayaran" class="form-input">
                <label for="kode_pembayaran" class="form-label">Silakan masukan kode pembayaran:</label>
                <input type="text" id="kode_pembayaran" name="kode_pembayaran" class="form-input" autofocus required>
                
                <label for="nama" class="form-label">Nama:</label>
                <textarea id="nama" name="nama" class="form-textarea" rows="1" cols="70" required readonly></textarea>
                
                <label for="tanggal" class="form-label">Tanggal:</label>
                <input type="date" id="tanggal" name="tanggal" accept="image/*" class="form-input" required>
                
                <input type="submit" name="cetak" value="Cetak" class="form-button">
            </form>
        </div>
        &nbsp;
        <div class="btn-box">
    ...
    function isiNamamu() {
        var kode_pembayaran = document.getElementById("kode_pembayaran").value;
        ...
        xhr.open("GET", "nama_get.php?kode_pembayaran=" + kode_pembayaran, true);
        xhr.send();
    }
    
    // Panggil fungsi isiNama saat input kode pembelian berubah
    document.getElementById("kode_pembayaran").addEventListener("input", isiNamamu);
</script>
<script>
    <?php if (isset($deleteSuccess) && $deleteSuccess) : ?>
        Swal.fire({
